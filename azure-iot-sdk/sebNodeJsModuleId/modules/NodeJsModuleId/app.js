...
// Using Module connection string
...
// TODO
var connectionString = 'HostName=<hubname>.azure-devices.net;DeviceId=<device name>;ModuleId=<module name>;SharedAccessKey=...';
...
client.on('error', function (err) {
  console.error(chalk.red('NodeJsModuleId connection error:\n' + err.toString()));
});
...
function onSetTelemetryInterval(request, response) {
  ...
  function directMethodResponse(err) {
    if(err) {
      console.error(chalk.red('NodeJsModuleId - An error ocurred when sending a method response:\n' + err.toString()));
    } else {
        console.log(chalk.green('NodeJsModuleId - Response to method \'' + request.methodName + '\' sent successfully.' ));
    }
  }
  ...
  if (isNaN(request.payload)) {
    ...
    response.send(200, 'NodeJsModuleId Telemetry interval set: ' + request.payload, directMethodResponse);
  }
}
...
function sendMessage(){
  ...
  console.log('NodeJsModuleId Sending message: ' + message.getData());

  // Send the message.
  client.sendEvent(message, function (err) {
    if (err) {
      console.error('NodeJsModuleId send error: ' + err.toString());
    } else {
      console.log('NodeJsModuleId message sent');
    }
  });
}
